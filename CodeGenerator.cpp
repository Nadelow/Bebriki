...
bool CodeGenerator::If()
{
	if (m_lines[m_j][1] == "IF") 
	{
		...
		{ //до булевского знака
			...
			int k = 0;
			while ((m_in_exp[k] != "=") and (m_in_exp[k] != ">") and (m_in_exp[k] != "<") and (m_in_exp[k] != "<>") and (m_in_exp[k] != ">=") and (m_in_exp[k] != "<="))
				k++;
...
void CodeGenerator::IfLeft()
{
	if (m_in_exp.size() > 1) 
	{
		while (true) 
		{
			while ((m_i < m_in_exp.size()) and (m_in_exp[m_i] != ")"))
				m_i++;
...
void CodeGenerator::IfLGenVars()
{
	while (true) 
	{
		...
		{
			if ((m_in_exp[m_i] == "*") or (m_in_exp[m_i] == "/") or (m_in_exp[m_i] == "^")) 
			{
				std::vector<std::string> help;
